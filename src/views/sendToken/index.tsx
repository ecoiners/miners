...
const SendTokenView: FC<SendTokenViewProps> = ({ setOpenSendToken }) => {
  ...
  // ✅ FIX: ambil token pakai connection.getParsedTokenAccountsByOwner()
  useEffect(() => {
    const fetchTokens = async () => {
      if (!publicKey) return;
      try {
        const acc = await connection.getParsedTokenAccountsByOwner(publicKey, {
          programId: TOKEN_PROGRAM_ID,
        });
        const data = acc.value.map((a) => ({
