	...
	const executePurchase = async () => {
		...
		if (!hasSufficientBalance) {
			...
			if (parseFloat(tokenBalance?.ecropBalance || "0") <= 20) {
				alert("Insufficient token supply, Please try again.");
			} else {
						...
						<div className={`${cardBg} ${cardBorder} backdrop-blur-sm rounded-xl border shadow-xl overflow-hidden transform transition duration-500 hover:shadow-2xl`}>
						  <div className="p-6 md:p-8">
									  ...
									  <button 
										...
										<button 
										  onClick={() => addTokenToMetamask()}
											className="w-full hidden lg:flex bg-gradient-to-r from-fuchsia-500 to-purple-600 hover:from-fuchsia-600 hover:to-purple-700 rounded-lg py-4 mb-4 items-center justify-center transition-all duration-300 font-medium shadow-lg"
										>
										  <img src="/logo.png" alt={tokenSymbol} className={"h-5 w-5"} />{" "}
											&nbsp;
								...
								<div className="flex flex-col space-y-2 text-xs">
								  <div className={`p-3 mb-1 rounded-lg ${isDarkMode ? "bg-gray-800/30": "bg-gray-100/70"}`}>
