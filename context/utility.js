...
export const handleTransactionError = (error, context="transaction", logToConsole=true) => {
	if (logToConsole) {
	...
	const isRejected = (error?.message && error.message.includes("user rejected")) || 
	  error.message.includes("rejected transaction") ||
		error.message.includes("User denied") ||
		error.message.includes("ACTION_REJECTED");
	
	if (isRejected || code === "ACTION_REJECED" || code === 4001) {
