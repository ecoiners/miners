...
// COMMAND: Ambil variabel environment untuk token
...
const HeroSection = ({isDarkMode, setIsReffelPopupOpeb}) => {
	...
	// COMMAND: Gunakan context Web3 untuk koneksi wallet dan smart contract
	const {
		account,
		isConnected,
		contractInfo,
	...
	// COMMAND: State untuk mengelola input dan seleksi token
	...
	// COMMAND: Refs untuk canvas animation dan particles
	const canvasRef = useRef(null);
	const particlesRef = useRef(null);
	const animationRef = useRef(null); // FIX: ganti useState jadi useRef
	
	// COMMAND: Fungsi untuk menghitung persentase progress penjualan token
	...
	// COMMAND: Memoized prices untuk performa yang lebih baik
	...
	// COMMAND: Effect untuk loading initial
	useEffect(() => {
	...
	// COMMAND: Effect untuk cek balance yang cukup
	useEffect(() => {
		...
		// COMMAND: Cek apakah balance token di bawah threshold
	...
	// COMMAND: Fungsi untuk menghitung jumlah token berdasarkan input
	...
	// COMMAND: Handler untuk perubahan input amount
	const handleInputChange = (value) => {
		setInputAmount(value);
		setTokenAmount(calculateTokenAmount(value, selectedToken));
	};
	
	// COMMAND: Handler untuk seleksi token
	const handleTokenSelection = (token) => {
		setSelectedToken(token);
		setTokenAmount(calculateTokenAmount(inputAmount, token)); // FIX: panggil calculateTokenAmount
	};
	
	// COMMAND: Fungsi untuk eksekusi pembelian token
	const executePurchase = async () => {
		
		if (!isConnected) {
			alert("Please connect your wallet first."); // FIX: typo "connected" jadi "connect"
			return;
		}
		...
		if (!hasSufficientBalance) {
			...
			if (parseFloat(tokenBalance?.ecropBalance || "0") <= 20) {
				alert("Insufficient token supply, Please try again."); // FIX: typo "agains" jadi "again"
			} else {
		...
		try {
			...
			// COMMAND: Reset amount setelah pembelian berhasil
			setInputAmount("0");
			setTokenAmount("0");
			
		} catch (error) {
			console.error(error);
			alert("Transaction failed, please try again😭😭"); // FIX: typo "agains" jadi "again"
		}
	};
	
	// COMMAND: Fungsi untuk mendapatkan balance saat ini berdasarkan token yang dipilih
	...
	// COMMAND: Fungsi untuk mendapatkan pesan button berdasarkan state
	...
	// COMMAND: Fungsi untuk mendapatkan icon token
	...
	// COMMAND: Variabel theme untuk dark/light mode
	const bgColor = isDarkMode ? "bg-[#0E0B12]" : "bg-[#F5F7FA]";
	const textColor = isDarkMode ? "text-white" : "text-gray-800";
	const secondaryTextColor = isDarkMode ? "text-gray-400" : "text-gray-600"; // FIX: typo "text-gray-40" jadi "text-gray-400"
	...
	// COMMAND: Fungsi untuk mendapatkan style button token
	...
	// COMMAND: Effect untuk canvas animation dengan particles
	useEffect(() => {
		...
		// COMMAND: Fungsi untuk resize canvas
		const resizeCanvas = () => {
			canvas.width = window.innerWidth;
			canvas.height = canvas.parentElement.offsetHeight; // FIX: typo "parrentElement" jadi "parentElement"
		};
		...
		// COMMAND: Inisialisasi particles
		...
		const animate = () => {
			...
			sortedParticles.forEach((particle) => {
				...
				const mouseInfluenceX = (mouseX - canvas.width / 2 - particle.x) * 0.0001; // FIX: typo "mouseInfluanceX" jadi "mouseInfluenceX"
				const mouseInfluenceY = (mouseY - canvas.height / 2 - particle.y) * 0.0001; // FIX: typo "mouseInfluanceY" jadi "mouseInfluenceY"
				...
				ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${opacity})`; // FIX: ganti jadi assignment =
				ctx.fill();
				
				if (distance > 0.8) {
					ctx.beginPath();
					ctx.arc(x2d, y2d, scaledSize * 1.5, 0, Math.PI * 2);
					ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${opacity * 0.3})`; // FIX: ganti jadi assignment =
					ctx.fill();
				}
	...
	return (
		// COMMAND: Container utama section
		<div className={`relative mt-12 w-full overflow-hidden ${bgColor}`}>
		  
		  {/* COMMAND: Background dengan efek visual */}
		  <div className={`absolute inset-0 z-0`}>
			  
				{/* COMMAND: Gradient overlay */}
				...
				{/* COMMAND: Canvas untuk particles animation */}
				<canvas
				  ref={canvasRef}
				...
				{/* COMMAND: Grid pattern background */}
				<div className="absolute inset-0 grid-pattern"></div>
				
				{/* COMMAND: Light rays effect */}
			...
			{/* COMMAND: Konten utama */}
			<div className="container mx-auto px-4 py-28 md:py-32 relative z-10">
			  <div className="flex flex-col md:flex-row items-center justify-between gap-12 md:gap-16">
				  
				  {/* COMMAND: Bagian kiri - Informasi token */}
				  <div className="w-full md:w-1/2 flex flex-col items-center md:items-start text-center md:text-left">
					 
					  {/* COMMAND: Badge presale live */}
					  <div className="inline-block p-2 px-4 rounded-full mb-6 bg-gradient-to-r from-green-400/10 to-indigo-500/10">
						  <p className="text-sm font-medium bg-clip-text text-transparent bg-gradient-to-r from-fuchsia-500 to-purple-600 animate-gradient-x"> {/* FIX: tambah "text-transparent" */}
							  Presale Live Now
							</p>
						</div>
						...
						{/* COMMAND: Deskripsi token */}
						<p className={`${secondaryTextColor} text-base md:text-lg max-w-md mb-8 leading-relaxed`}> {/* FIX: typo "md-text-lg" jadi "md:text-lg" */}
						  Revolutionizing intelligence through decentralized innovation.
							Join E100 the future of blockchains technology today.
						</p>
						
						{/* COMMAND: Fitur-fitur utama */}
                ...
                >
                  ...
                  />
					...
					{/* COMMAND: Bagian kanan - Card pembelian token */}
					<div className="w-full md:w-1/2 max-w-md mx-auto relative">
					  ...
					  {isLoading && (
						...
						{/* COMMAND: Card utama untuk pembelian */}
						<div className={`${cardBg} ${cardBorder} backdrop-blur-sm rounded-xl border shadow-xl overflow-hidden transform transition duration-500 hover:shadow-2xl`}>
						  <div className="p-6 md:p-8">
								...
								{/* COMMAND: Notifikasi jika token tidak ditemukan di wallet */}
								...
								{/* COMMAND: Informasi harga saat ini dan berikutnya */}
								...
								{/* COMMAND: Progress bar penjualan */}
								...
								{/* COMMAND: Seleksi token untuk pembayaran */}
								...
								{/* COMMAND: Display balance */}
								<div className={`text-sm py-2 mb-6 px-4 rounded-lg text-center ${secondaryTextColor} ${isDarkMode ? "bg-gray-800/30": "bg-gray-100/70"}`}> {/* FIX: typo "bg-gray-100/700" jadi "bg-gray-100/70" */}
								...
								{/* COMMAND: Input amount untuk pembayaran dan penerimaan */}
								...
								{isConnected ? (
									...
									// COMMAND: Connect wallet button jika belum terkoneksi
									<CustomConnectButton childStyle="w-full mb-4 py-4 rounded-lg gap-2 font-medium flex items-center justify-center" />
								)}
								
								{/* COMMAND: Help links */}
								<div className="flex flex-col space-y-2 text-xs">
								  <div className={`p-3 mb-1 rounded-lg ${isDarkMode ? "bg-gray-800/30": "bg-gray-100/70"}`}> {/* FIX: typo "bg-gray-100/700" jadi "bg-gray-100/70" */}
			...
			<style jsx>
			  {`
				  @keyframes pulse-slow {
						...
						100% {
							opacity: 0.8;
							transform: scale(1); // FIX: tambah : setelah transform
						}
						50% {
							opacity: 1;
					...
					.animated-progress {
						animation: progress 1.5s ease-out; // FIX: hapus ! jadi ;
					}
					...
					.grid-pattern {
						...
						animation: pulse-grid 3s ease-in-out infinite alternate; // FIX: tambah 3s dan nama keyframes
					}
					...
					.light-ray { // FIX: tambah . di depan light-ray
						position: absolute;
						width: 200%;
						height: 100%; // FIX: typo "100" jadi "100%"
					...
					.ray3 {
						animation: moveRay 25s linear 10s infinite; // FIX: tambah "moveRay" sebelum 25s
					}
					...
					@keyframes shimmer {
						...
						100% {
							background-position: 200% 0; // FIX: typo "backgrounr-position" jadi "background-position"
						}
					}
				`}
			</style>
